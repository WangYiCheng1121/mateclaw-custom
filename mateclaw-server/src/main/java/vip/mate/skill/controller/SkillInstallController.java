...
/**
 * Skill 安装管理接口（客户端侧 — 只读）
 * <p>
 * 客户端仅保留 ClawHub 市场搜索和安装任务状态查询能力。
 * 实际安装/卸载操作已迁移至平台端统一管理。
 * <p>
 * 已移除的接口（迁移至平台端）：
 * - POST   /api/v1/skills/install/start     → 开始安装
 * - POST   /api/v1/skills/install/cancel/{taskId} → 取消安装
 * - POST   /api/v1/skills/install/upload     → ZIP 上传安装
 * - DELETE /api/v1/skills/install/{skillName} → 卸载
 *
 * @author WYC
 */
@Tag(name = "技能安装")
...
//    }
    ...
    @Operation(summary = "查询安装任务状态（查看平台端发起的安装进度）")
