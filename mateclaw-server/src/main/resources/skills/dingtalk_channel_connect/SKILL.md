...
version: "1.3.0"
optional: true
description: "使用可见浏览器自动完成 GLClaw 钉钉渠道接入。遇到登录页必须暂停等待用户手动登录后继续。"
dependencies:
  tools:
    - browser_use
...
通过可见浏览器自动化完成钉钉应用创建与 GLClaw 渠道绑定。
...
2. 再用文件上传操作（GLClaw browser_use 支持 file input 的 `type` 操作传入路径）
...
3. 引导用户在 GLClaw 控制台绑定：

**方式 A — 控制台前端：**
> 进入 GLClaw 管理界面 → 渠道 → 新建渠道 → 选择钉钉 → 填入 Client ID 和 Client Secret
...
**Agent 不主动修改 GLClaw 配置文件，只引导用户操作。**
