...
- Not a 30-node Dify-style orchestrator — GLClaw workflows stay deliberately minimal: **a linear array of steps with one `mode` field expressing the control flow**
...
- `actingAgentId`: for `sequential` and the three GLClaw modes → that step's agent; for other modes → publisher as fallback
