...
- **Ollama** — local GPU inference; GLClaw auto-detects it on `localhost:11434` at startup
...
**You also don't need to set any API key as an environment variable to start GLClaw.** All provider configuration is done through the UI at `Settings → Models` after startup.
...
2. Sign up or log in
3. Create an API key
4. In GLClaw, go to `Settings → Models → DashScope` and paste it
...
GLClaw implements **sliding-window renewal** — when a token is within 25% of expiry, the server issues a new one in the `X-New-Token` response header. The frontend handles this automatically.
