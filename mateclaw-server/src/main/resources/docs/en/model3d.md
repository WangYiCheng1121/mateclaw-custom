...
Text-to-3D and image-to-3D in one tool call. Configure the credentials once and the agent can call `model3d_generate` to produce a `.glb` model that lands directly in the chat bubble ‚Ä?interactive preview, drag to rotate.
...
| **Latency** | 1‚Ä? minutes (Pro slower, Rapid faster) |
...
1. Open **[Cloud Access Management ‚Ü?API Keys](https://console.cloud.tencent.com/cam/capi)**
2. Click **Create Key**. Tencent gives you both:
   - `SecretId` (starts with `AKID`, ~36 chars)
   - `SecretKey` (~32 chars)
3. **Save both** ‚Ä?the SecretKey is shown only once and cannot be retrieved later.

::: tip About `sk-xxx` keys from "API Key ÁÆ°ÁêÜ"
...
Some variants (notably `HY-3D-3.1`) may require a separate quota application or paid plan ‚Ä?check the console's quota dashboard.

## 3. Configure credentials in GLClaw
...
Go to **Settings ‚Ü?3D Generation**:
...
Speak naturally ‚Ä?the agent picks `model3d_generate` automatically:
...
```
Quickly generate a 3D model of a red apple    ‚Ü?LLM selects HY-3D-Express
```

```
Generate a 3D model from this image: https://example.com/foo.png    ‚Ü?image-to-3D
```

```
Generate a white-model 3D (no textures): a mechanical gear    ‚Ü?Geometry mode
```
...
3. **1‚Ä? minutes later** the `async_task_completed` SSE event lands in the conversation.
4. **`<model-viewer>` renders the `.glb`** inline ‚Ä?drag, rotate, zoom.
...
\* Either `prompt` or `imageUrl` is required (XOR ‚Ä?Pro doesn't accept both, except in Sketch mode which is not exposed yet).

---

## Troubleshooting

### 1. `3D Ê®°ÂûãÁîüÊàêÂäüËÉΩÊú™ÂêØÁî®ÔºåËØ∑Âú®Á≥ªÁªüËÆæÁΩÆ‰∏≠ÂºÄÂêØ` / "3D generation is not enabled"

‚Ü?The feature toggle is off. Go to **Settings ‚Ü?3D Generation** and enable it.

### 2. `Provider api_key must be "SecretId:SecretKey" (colon-joined)`

‚Ü?Wrong credential format. You probably saved one of:
- A single SecretId (no SecretKey appended)
- A single SecretKey
- A single `sk-xxx` token (that's a TokenHub key, not for 3D)
- Used a space instead of `:`

Correct: `AKIDxxxx...:zzzz...` ‚Ä?exactly one ASCII colon, no whitespace.

### 3. `ResourceInsufficient` (ËµÑÊ∫ê‰∏çË∂≥)

‚Ü?Tencent-side business error. Possible causes:
...
‚Ü?Tencent couldn't fetch your `imageUrl`. The URL must be reachable from the public internet ‚Ä?`localhost`, internal IPs, and signed URLs that have expired won't work. Confirm:
- The URL opens directly in an incognito browser window
- File type is `jpg/png/jpeg/webp`, resolution 128‚Ä?000px per side, ‚â?MB

### 5. Task hangs for 15 minutes

‚Ü?The worker times out at 15 min by default. Check backend logs:
...
‚Ü?Tencent returned an OBJ bundle (.zip with OBJ + textures + MTL) instead of a single GLB. Our code prefers GLB (`pickBestResultFile`), but if Tencent only returns OBJ for that request, the frontend falls back to a download link. **Defaulting to `HY-3D-3.1` usually yields a GLB.**
...
```
[ user ] ‚îÄ‚îÄ natural language ‚îÄ‚ñ?[ agent ] ‚îÄ‚ñ?model3d_generate
                                                ‚î?
                                  (route on model field)
                                                ‚î?
                ‚îå‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚î¥‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚î?
                ‚ñ?                                             ‚ñ?
        SubmitHunyuanTo3DProJob               SubmitHunyuanTo3DRapidJob
        (HY-3D-3.1 / HY-3D-3.0)                (HY-3D-Express)
                ‚î?                                             ‚î?
                ‚îî‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ ai3d.tencentcloudapi.com ‚îÄ‚îÄ‚îÄ‚îÄ‚î?
                                                ‚î?
                                  returns JobId (24-h URL)
                                                ‚î?
            AsyncTaskService polls Query{Pro,Rapid}HunyuanTo3DJob every 8 s
                                                ‚î?
                              status ‚Ü?DONE? ‚îÄ‚ñ?ResultFile3Ds[]
                                                ‚î?
                            ...
                            broadcast SSE async_task_completed
                                                ‚î?
                ‚ñ?
        frontend useChat detects modelUrl ‚îÄ‚ñ?MessageBubble bridges virtual attachment
                                                ‚î?
...
‚Ä?‚Ä? minutes‚Ä?
[Model3dDownloader] Downloading 3D model from https://hunyuan-prod-‚Ä?cos.../...glb to data/chat-uploads/.../model_d73723f14c7c4167.glb
