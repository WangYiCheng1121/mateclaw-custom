...
GLClaw's LLM Wiki does something different. Drop raw material into a knowledge base and the system reads it, digests it, and writes structured Wiki pages — each with a summary, backlinks, and provenance pointers back to the source passage. You can open any page and read it. You can edit it. Agents read summaries automatically and pull full pages on demand.
...
GLClaw's LLM Wiki **is the same idea, raised into a product**:
...
- Not an isolated tool — the **knowledge layer of GLClaw's agent operating system**, threaded through memory, agents, and channel delivery
...
- **Scan a local directory** — desktop only. Point at a folder and GLClaw walks it recursively, respecting `.gitignore`, importing everything that looks like text.
