---
title: MCP Integration ‚Ä?Model Context Protocol Tool Extension
...
# MCP ‚Ä?Model Context Protocol

**MCP is how GLClaw talks to tools someone else built.**

Model Context Protocol is an open standard from Anthropic for connecting AI models to external tools and data. An MCP server is a process ‚Ä?local or remote ‚Ä?that advertises a set of tools over JSON-RPC. GLClaw acts as an MCP *client*: it connects, discovers tools via `tools/list`, and exposes them to your agents as if they were native. **From the agent's point of view, there's no difference between a built-in `@Tool` Spring bean and a tool coming from an MCP server.**

This is the escape hatch. If you need a capability GLClaw doesn't ship with ‚Ä?filesystem access for a sandboxed directory, Tavily search, a custom internal data service, a browser automation suite ‚Ä?there's probably already an MCP server for it, and you can plug it in without writing a line of Java.
...
```
‚îå‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚î?             ‚îå‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚î?
‚î?    GLClaw           ‚î?             ‚î?    MCP Server        ‚î?
‚î?    (MCP Client)       ‚î?             ‚î?    (Tool Provider)   ‚î?
‚î?                      ‚î?  JSON-RPC   ‚î?                      ‚î?
‚î? Agent Engine  ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îº‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îº‚îÄ‚îÄ‚ñ?Tool A             ‚î?
‚î?                      ‚î?             ‚î?   Tool B             ‚î?
‚î? Tool Registry ‚óÑ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îº‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îº‚îÄ‚îÄ‚îÄ Tool Discovery     ‚î?
‚î?                      ‚î?             ‚î?   (tools/list)       ‚î?
‚îî‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚î?             ‚îî‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚î?
```

Core concepts:

- **MCP Client** ‚Ä?GLClaw, connecting to servers, discovering tools, forwarding invocations
- **MCP Server** ‚Ä?a third-party process declaring its available tools and executing calls
- **Tool Discovery** ‚Ä?the client sends `tools/list` to retrieve every tool and its parameter schema
- **Tool Invocation** ‚Ä?when the agent decides to call a tool, the client forwards the request to the right MCP server
...
```
GLClaw  ‚îÄ‚îÄ stdin ‚îÄ‚îÄ‚ñ? MCP Server subprocess
          ‚óÑ‚îÄ stdout ‚îÄ‚îÄ
```
...
```
GLClaw  ‚îÄ‚îÄ HTTP POST ‚îÄ‚îÄ‚ñ? Remote MCP Server
          ‚óÑ‚îÄ HTTP Stream ‚îÄ‚îÄ
```
...
`Tools ‚Ü?MCP Servers ‚Ü?Add MCP Server`. Fill in:

- **Name** ‚Ä?unique identifier (letters, numbers, `_`, `-`, `.`, spaces; 1‚Ä?28 chars)
- **Description** ‚Ä?optional
- **Transport type** ‚Ä?`stdio`, `streamable_http`, or `sse`
- **Command** (stdio) ‚Ä?`npx`, `node`, `python`, etc.
- **Arguments** (stdio) ‚Ä?JSON array (e.g., `["-y", "@anthropic/mcp-filesystem", "/path"]`)
- **Working directory** (stdio) ‚Ä?optional
- **Environment variables** (stdio) ‚Ä?JSON object; supports `${ENV_VAR}` references
- **URL** (streamable_http/sse) ‚Ä?server endpoint
- **HTTP Headers** (streamable_http/sse) ‚Ä?JSON object (e.g., `{"Authorization": "Bearer token"}`)
- **Connect timeout** ‚Ä?default 30s
- **Read timeout** ‚Ä?default 30s

Save. If enabled, GLClaw auto-attempts to connect and discover tools.

### Testing, enabling, status

- **Test Connection** ‚Ä?sends `tools/list`, returns result, latency, tool list
- **Enable/Disable toggle** ‚Ä?drop connection without deleting config
- **Status** ‚Ä?`connected` / `disconnected` / `error` with error detail
...
### Example 1 ‚Ä?Filesystem MCP (stdio)
...
### Example 2 ‚Ä?Remote HTTP with auth
...
### Example 3 ‚Ä?Tavily search (stdio + env vars)
...
Iterate enabled MCP servers
   ‚î?
   ‚ñ?
Connect by transport ‚Ü?initialize ‚Ü?list tools ‚Ü?cache
   ‚î?
   ‚ñ?
...
**Key:** the agent fetches the **latest** active tool list on every invocation, so adding or removing MCP servers takes effect **without restarting**. From the agent's perspective, **MCP tools and built-in tools are identical** ‚Ä?no difference.
...
::: tip New in 1.3.0
Before v1.2.0, all employees could call every MCP tool by default ‚Ä?it was a global switch. v1.3.0 makes the binding **per-employee**, and adds dirty-state detection plus namespace collision handling.
:::

### Three problems it solves

**Problem 1: Tool namespace collisions.**
Two MCP servers both expose `read_file` ‚Ä?which one wins? v1.3.0 internally uses a **stable server-prefixed callback name** (`{serverName}__{toolName}`) and persists it to `mate_mcp_server.cached_tools`. The picker shows them as `serverA__read_file` and `serverB__read_file`; the agent's prompt maps them back to original names to save tokens and avoid LLM confusion.

**Problem 2: MCP server / tool rename breaks bindings.**
In v1.2.0, renaming a server orphaned every employee bound to it. v1.3.0 introduces a **persistent tool cache**: every successful list-tools writes tool metadata to a `cached_tools` JSON column on `mate_mcp_server`. When validating bindings and the server is temporarily unreachable, the cache is consulted as fallback ‚Ä?bindings stay marked `stale` and become live again the moment the server reconnects.
...
|---|---|---|
| `connected` | Server online, tool visible | ‚ú?Persist normally |
| `stale` | Server temporarily offline but in cache | ‚ú?Persist (marked stale) |
| `unavailable` | Server disabled | ‚ú?Persist (marked unavailable) |
| `orphan` | Server / tool no longer exists at all | ‚ù?Reject save, prompt user to clear |

### Where to see tool status

`Agents ‚Ü?pick employee ‚Ü?Tools` ‚Ä?see [Agent tool binding](./agents#tool-binding-per-agent-tool-picker).
...
- `AgentBindingService.getEffectiveToolNames(agentId)` is the single source of truth for tool dispatch ‚Ä?runs every turn, ensuring the editor view and the runtime view always agree

### Server-side rules

- MCP servers bridged in via ACP **cannot** be edited from the MCP server list (they're owned by the ACP server's own lifecycle)
- A tool marked `unavailable` is **not listed** in the agent's system prompt ‚Ä?the LLM won't reach for it, but the binding row is preserved
- `returnDirect=true` tools (whose output replaces the assistant turn) go through the same ACL ‚Ä?they **do not bypass** binding
...
- `last_status` ‚Ä?`connected` / `disconnected` / `error`
- `last_error` ‚Ä?error message
- `last_connected_time` ‚Ä?timestamp of last success
- `tool_count` ‚Ä?currently discovered tools
...
## Database storage ‚Ä?`mate_mcp_server`

| Column | Type | Default | Purpose |
|--------|------|---------|---------|
| `id` | BIGINT | ‚Ä?| Primary key |
| `name` | VARCHAR(128) | ‚Ä?| Unique identifier |
...
| `create_time` / `update_time` | DATETIME | ‚Ä?| Timestamps |
...
- `${VAR_NAME}` ‚Ä?exact match and replacement
- `$VAR_NAME` ‚Ä?regex match
...
- [Tools](./tools) ‚Ä?how MCP tools relate to built-in tools
- [Skills](./skills) ‚Ä?MCP-backed skills
- [Configuration](./config) ‚Ä?full configuration reference
