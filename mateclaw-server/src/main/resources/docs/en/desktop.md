...
Frontend assets can be **hot-updated independently** �?a frontend-only fix doesn't require a new installer. See `GLClaw-desktop/scripts/` and `desktop-ui-hot-update.md` for the hot-update build flow.
...
| Setting | Purpose |
|---------|---------|
| `appId` | `vip.mate.GLClaw` �?system registration and code signing |
...
- **macOS** �?right-click �?**Open** to bypass Gatekeeper (first launch). Production: Apple Developer certificate + notarization. See `GLClaw-desktop/CODESIGNING.md`.
...
- First launch takes 10�?0 seconds (database init)
...
- Bundled JRE makes the installer 80�?20 MB
