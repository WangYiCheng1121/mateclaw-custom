...
A channel in GLClaw is a different door into the same agent. Your team chats in Feishu? Put an agent in Feishu. Someone prefers Telegram? Same agent, same memory, in Telegram. Web console for the operators, DingTalk for the field, Slack for engineering — one deployment, nine doors.
...
This is why GLClaw channels don't sit silent after a hiccup: they come back on their own.
...
DingTalk's OAuth Device Flow. GLClaw requests a device code from `oapi.dingtalk.com`, encodes it as a QR; once you confirm, credentials land in the form via the polling endpoint. **No webhook, no public IP required.**
:::
...
::: tip What's happening under the hood
This goes through the QQ Open Platform Lite portal: GLClaw mints a temporary session and the credentials land in the form via an AES-256-GCM encrypted exchange. The session is valid for 12 minutes and auto-invalidates on expiry. **No hand-copied credentials anywhere in the flow.**
:::
...
- **Webhook mode needs HTTPS.** Production deployments should front GLClaw with Nginx + SSL.
