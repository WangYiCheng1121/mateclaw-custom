...
- `agent_lifecycle` â†?agent (optional) + phase: `spawned` / `terminated` / `crashed` (optional)
...
Before v1.3.0 GLClaw already had a standalone cron subsystem (`mate_cron_job` table + `CronJobService`). The trigger system **does not replace it** â€?
...
- **`agent_lifecycle` granularity is `spawned` / `terminated` / `crashed`** â€?not "started / completed / failed" per step
