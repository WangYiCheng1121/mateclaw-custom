...
GLClaw 里的一个渠道是通往同一个 Agent 的另一扇门。团队在飞书里沟通？把 Agent 放进飞书。有人喜欢用 Telegram？同一个 Agent，同一份记忆，出现在 Telegram。Web 控制台给运维，钉钉给现场，Slack 给工程——**一次部署，九扇门**。
...
这就是为什么 GLClaw 的渠道不会在一次抖动之后永远静默：**它们自己会回来。**
...
::: tip 它在做什么
背后走钉钉的 OAuth Device Flow——GLClaw 在 `oapi.dingtalk.com` 申请一个设备码，编成 QR；你确认后，凭证从轮询接口落地到表单。**整个过程没有 webhook，没有公网 IP 要求。**
:::
...
::: tip 它在做什么
背后走 QQ 开放平台的精简版（Lite）授权门户：GLClaw 生成一个临时会话，凭证通过 AES-256-GCM 加密交换落地到表单。会话 12 分钟内有效，过期自动失效。**整个过程没有手抄凭证这一步。**
:::
...
- **Webhook 模式需要 HTTPS。** 生产部署应该用 Nginx + SSL 挡在 GLClaw 前面。
