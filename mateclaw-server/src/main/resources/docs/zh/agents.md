...
description: GLClaw 的多智能体系统支持 ReAct 推理循环和 Plan-and-Execute 任务拆解两种模式。Agent 之间可以互相委派，实现真正的多智能体协作。
head:
  - - meta
    - name: keywords
...
注意一个**没有**的东西：模型。整个 GLClaw 部署里只有**一个全局默认模型**（在 `设置 → 模型` 里设置），所有 Agent 在运行时用的都是它。Agent 行上那个 `model_name` 字段是历史遗留——**被忽略**。这是刻意的：换模型是整个部署一次点击的事，不是三十次。
...
不是所有问题都值得深度推理，但有些问题需要。GLClaw 支持按 Agent、按对话打开深度思考模式：
