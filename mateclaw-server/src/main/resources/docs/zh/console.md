...
控制台是每个 GLClaw 部署都自带的 Vue 3 SPA。它跑在你的浏览器里（或者 Electron 桌面窗口里），通过 REST + SSE 跟 Spring Boot 后端对话，把 GLClaw 的每一项能力——聊天、Agent、知识、工具、技能、渠道、安全、定时任务、用量分析——都放在**同一套登录**后面。
...
GLClaw 用领域驱动的 Pinia store。每个 store **独占**自己的状态。
