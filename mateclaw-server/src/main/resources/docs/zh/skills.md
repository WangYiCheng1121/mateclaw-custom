...
技能装进来不一定能用——它可能需要某个 API key、某个 CLI 工具、某个 GLClaw 开关被打开。
...
ACP（Agent Client Protocol）是一种把外部 Agent 客户端（Claude Code、Codex、其他兼容客户端）以技能身份接入 GLClaw 的协议。
