...
> 基于 GLClaw 内置的 3 个 Agent、19 个工具、31 条 Guard 规则和审批工作流设计。
> 默认登录：admin / admin123
...
- **Agent**: GLClaw Assistant（ReAct），绑定 WebSearch 工具
- **操作**: 发送 "搜索一下今天的科技新闻"
- **预期**:
...
- **Agent**: GLClaw Assistant（ReAct），绑定 Shell 工具
...
### TC-2.2 文件写入触发审批
- **Agent**: GLClaw Assistant（ReAct），绑定 WriteFile 工具
...
- **操作**: 发送 "帮我调研 GLClaw 项目的技术栈，列出前端和后端分别用了哪些核心技术，然后生成一个技术概览文档"
- **预期**:
...
### TC-4.1 浏览器操作时间线默认折叠
- **Agent**: GLClaw Assistant（ReAct），绑定 BrowserUse 工具
...
### TC-8.1 普通对话流程（无工具调用）
- **Agent**: GLClaw Assistant
- **操作**: 发送 "你好，介绍一下你自己"
- **预期**: 正常生成回复，无 UI 异常

### TC-8.2 多轮对话 + 工具调用
- **Agent**: GLClaw Assistant，绑定多个工具
